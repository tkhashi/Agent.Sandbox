...
2. **ハッチング(`_detect_hatching`)**: 軸並行の`line`(`curve`は対象外)を対象に、`geometry.find_evenly_spaced_runs`で同一線幅の面が4本以上ほぼ等間隔に並ぶ区間を検出する。線幅グループ内の面の長さ分布から、自然な倍率ジャンプ(`split_by_relative_jump`)で「短いノイズ」集団を**チェーン構築前に**除いた上で座標順に連結する(断熱材の蛇腹記号(短いティックマーク)や家具・柱の輪郭線が、本物のハッチング線の間に挟まってチェーンを分断してしまう問題への対策)。連結後もさらに、各面の長さがランの間隔(中央値)に対して十分長い(既定5倍以上)ものだけを残す事後フィルタを重ねて適用する。線幅が一致し、間隔が`geometry.cluster_by_relative_jumps`(自然な倍率ジャンプによる再帰分割、詳細はADR0010追記)で同一クラスタと判定されたインスタンス同士を同一グループに統合する(離れた場所にある同じパターンも統合対象)
...
- **ドア検出**: `find_arcs`は壁分類(ADR0008)と同じ限界を持ち、本PDFでは実際のドア記号を検出できず装飾的な小さい円弧のみが検出される。半径・近接度で密集する装飾クラスタ(断熱材の蛇腹記号・角丸四角の角等)は除外しているが(ADR0010追記参照)、除外後に残る弧が本物のドアである保証はない
- **ハッチング検出**: チェーン構築前の事前フィルタ(線幅グループ内の長さ分布の自然な倍率ジャンプ)が機能しない線幅・軸の組み合わせでは検出漏れが起きる(実データ: 収納・トイレ付近の水平方向ハッチングは、線幅グループ内の長さがなだらかに連続分布しており明確な境界が無いため検出できていない)。事後フィルタの倍率・グループ化の倍率ジャンプ閾値も経験的な値であり、他のPDFでの再調整が必要になる可能性がある
- **点線閉ループ検出**: 密度の高い領域では、複数の独立したアイコンが1つの大きな連結成分として検出されることがある(完全にきれいに分離しきれていない)。関連する閾値(`_MAX_DASH_LENGTH_PAGE_RATIO`, `_DASH_GAP_TO_LENGTH_RATIO`)は経験的な値

詳細は`docs/adr/0010-unread-info-classification.md`を参照。
